...
## Goal
Implement and verify SPI bring-up for W5100S using WIZnet `ioLibrary_Driver`.
The firmware keeps a UART CLI for chip/register checks, but register access is delegated to `ioLibrary_Driver`.
...
- `ioLibrary_Driver` callback registration for W5100S SPI access
- UART CLI commands for chip/register access through `ioLibrary_Driver`
...
## Limitation
Without actual module attached, register values are undefined (often `0xFF`),
but CLI command handling remains verifiable.
